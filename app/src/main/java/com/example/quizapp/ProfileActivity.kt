...
class ProfileActivity : AppCompatActivity() {
        ...
        dbf.addValueEventListener( object: ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if(userinfo!=null){
                    ...
                    email.editText?.setText(userinfo!!.userEmail)
    ...
    private fun init(){
        ...
        email = findViewById(R.id.database_userEmail)
    ...
    private fun updateProfileData(profile: String) {
        ...
        if(TextUtils.isEmpty(email.editText?.text)){
            ...
            map["userEmail"] = email.editText?.text.toString()
            ...
            dbf.updateChildren(map).addOnSuccessListener {
                ...
                val dbCollection : CollectionReference = FirebaseFirestore.getInstance().collection("score_info")
                dbCollection.document(id).addSnapshotListener { value, error ->
                    run {
                        if (value?.get("score") != null) {
                            FirebaseFirestore.getInstance().runTransaction { transaction ->

                                transaction.update(dbCollection.document(id), "imgSrc", profile)
                                transaction.update(dbCollection.document(id), "username", name.editText?.text.toString())
